...
from PySide6.QtWidgets import QPushButton, QCheckBox, QLabel, QGraphicsDropShadowEffect
from PySide6.QtGui import QPainter, QColor, QBrush, QPainterPath, QFont, QLinearGradient
...
class GlowLabel(QLabel):
    def __init__(self, text="", parent=None, font_size=54):
        super().__init__(text, parent)
        
        # 1. Configuración de transparencia y limpieza de bordes
        self.setAttribute(Qt.WA_TranslucentBackground)
        self.setStyleSheet("background: transparent; border: none;")

        # 2. Fuente Base
        font = QFont("Segoe UI", font_size, QFont.ExtraBold)
        self.setFont(font)
        
        # --- TUS COLORES (Stock Flow Palette) ---
        self.colors = [
            QColor("#f7a51b"), # Inicio: Tu Naranja Corporativo
            QColor("#f7a51b"), # Medio: Blanco (Núcleo de energía)
            QColor("#f7a51b")  # Fin: Dorado suave
        ]
        
        # --- EL GLOW (Del primer código, pero color naranja) ---
        self.glow = QGraphicsDropShadowEffect(self)
        self.glow.setBlurRadius(25)     # Radio original que funcionaba bien
        self.glow.setColor(QColor("#f7a51b")) # Glow naranja
        self.glow.setOffset(0, 0)
        self.setGraphicsEffect(self.glow)
        
        # Márgenes originales (10) para asegurar la geometría
    ...
    def paintEvent(self, event):
        ...
        # 2. Gradiente Horizontal (TopLeft -> TopRight)
        # (Usamos la dirección del primer código que te gustó)
        ...
        # --- FÓRMULA MATEMÁTICA ORIGINAL (La "Amazon") ---
        fm = self.fontMetrics()
        text_width = fm.horizontalAdvance(self.text())
        text_height = fm.ascent() # Usamos ascent como altura base
        
        # Centrado X
        x = (rect.width() - text_width) / 2
        
        # Centrado Y (Esta es la fórmula exacta del primer código)
