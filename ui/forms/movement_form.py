...
class MovementsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.btn_adjust.clicked.connect(
            lambda: self.set_movement_type("ADJUST"))
    ...
    def _filter_origin_locations(self, item_id):
        ...
        if locations_list:
            for loc in locations_list:
                ...
                self.combo_origin.addItem(
                    display_text, userData=loc['id_location'])
        else:
    ...
    def _update_info_panel(self):
        ...
        if not item_id:
            ...
            self.lbl_current_stock.setStyleSheet(
                "font-weight: bold; font-size: 16px; margin-top: 10px; color: #B0BEC5;")
            return
        ...
        self.lbl_current_stock.setStyleSheet(
            f"font-weight: bold; font-size: 24px; margin-top: 10px; color: {stock_color};")
    ...
    def _on_save(self):
        ...
        if not id_item_resolved:
            QMessageBox.warning(
                self, "Item no válido", "Por favor selecciona un item de la lista sugerida.")
            return
        ...
        if mov_type == "IN":
            ...
            if not origin_id:
                QMessageBox.warning(
                    self, "Faltan datos", "Para un Ajuste debe seleccionar el Origen.")
                return
            ...
            if not dest_id:
                if reason_key == "relocation":
                    QMessageBox.warning(
                        self, "Faltan datos", "Para una 'Reubicación', la Ubicación Destino es obligatoria.")
                    return
                ...
                # CASO B: Si es Scrap/Daño, advertir que se eliminará (Hard Delete)
                    ...
                    reply = QMessageBox.question(
                        self, "Confirmar Baja", msg, QMessageBox.Yes | QMessageBox.No)
        ...
        try:
            ...
            QMessageBox.information(
                self, "Éxito", f"Movimiento registrado con ID {new_id}")
            self._on_clear()

        except Exception as e:
            QMessageBox.critical(
                self, "Error de Base de Datos", f"No se pudo registrar:\n{e}")
    ...
    def _on_clear(self):
        ...
        if self.combo_origin.isEnabled():
            self.combo_origin.setCurrentIndex(0)
        if self.combo_dest.isEnabled():
            self.combo_dest.setCurrentIndex(0)
