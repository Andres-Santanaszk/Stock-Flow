# Crear las tablas en la DB ya creada con create_db.py
...
DEFAULT_FILES = [
    ...
    "003_catalogs.sql",
    "004_inventory.sql",
    # agrega aquí "090_views.sql", "095_functions.sql", "100_triggers.sql", etc. si los creas
]

SEED_FILE = "999_seed.sql"  # opcional


def run_sql_file(conn, path):
    with open(path, "r", encoding="utf-8") as f, conn.cursor() as cur:
        sql_text = f.read()
        # psycopg2 soporta múltiples sentencias en un execute
...
def migrate(load_seed=True):
    conn = psycopg2.connect(
        ...
        dbname=DB_NAME,   # ya debe existir
        host=DB_HOST,
        port=DB_PORT,
        connect_timeout=5,
...
if __name__ == "__main__":
    # permite desactivar seed con variable de entorno
